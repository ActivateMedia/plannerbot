...
PlannerBot.prototype._onStart = function () {
    ...
    if(typeof users !== "undefined" &&
       ...
       users._value.members.forEach(function(user, index) {
        ...
        self.slack.members[user.id] = {"username" : user.name, "first_name": first_name};
        console.log(user.name.white.bold + " [" + first_name.yellow + "]");
...
PlannerBot.prototype._onMessage = function (message) {
    ...
    if (this._isChatMessage(message) &&        
      ...
      console.log("\n                                   \n".bgCyan);
      console.log(self.slack.members[message.user].first_name.white.bold + ": " + message.text.white);
      ...
      self._botTyping(event.key);    
      
      var _isPvt = self._isPrivateConversation(message);
      
      console.log(colors.white("isPrivate = " + _isPvt));
      
      self._getIntent(event, message.text, function(recognised, output) {
        if(recognised === true) {          
          for (var i = 0; i < output.outcomes.length; i++) {
            var intent = output.outcomes[i].intent;

            console.log("Message classified as: " + intent.white.bold);
            if(intent === "Hello" && _isPvt) {
              self._intentHello(message);
            } else if(intent === "Help") {
              self._intentHelp(message);
            } else if(intent === "Stop_Schedule") {
              self._intentStop(event, message);
            } else if(intent === "Schedule") {
              event.state = "pending_scheduling";
              self._intentSchedule(event, message);
            } else if(intent === "Insult" && _isPvt) {
              self._intentInsult(message);
            } else if(intent === "Bot_Name" && _isPvt) {
              self._intentBot_Name(message);
            } else if(intent === "Want_to_Schedule") {
              event.state = "want_to_schedule";
              self._intentWant_to_Schedule(event, message);
            } else if(_isPvt) {
              self._intentOther(message);
            }
          }
        } else {
          console.log("Intent not recognised, maybe an error in the API call?");
          console.log(output);
        }        
      });
 ...
 *
 */
PlannerBot.prototype._intentStop = function (event, message) {
  var self = this;
  var _msg = "ok, got it";
 ...
 *
 */
PlannerBot.prototype._intentInsult = function (message) {
  var self = this;
  var answers = ['Stop insulting me, I\'ll tell Andrea', 'So rude.', 'You are not very nice with me', 'I\'m just a bot'];
  var _msg = answers[Math.floor(Math.random()*answers.length)];
  self._replyToUser(message.user, _msg);   
}
 ...
 *
 */
PlannerBot.prototype._intentBot_Name = function (message) {
  var self = this;
  var answers = ['My name is Bot, Planner Bot.', 'I\'m PlannerBot'];
  var _msg = answers[Math.floor(Math.random()*answers.length)];
  self._replyToUser(message.user, _msg);   
}
 ...
 *
 */
PlannerBot.prototype._intentHelp = function (message) {
  var self = this;
...
/*
 * Intent: Want_to_Schedule
 *
 */
PlannerBot.prototype._intentWant_to_Schedule = function (event, message) {
  var self = this;
  var _msg = self.slack.members[message.user].first_name + ", tell me the name of the event that you want to schedule";
  self._replyToUser(message.user, _msg);   
 ...
 *
 */
PlannerBot.prototype._intentOther = function (message) {
  var self = this;
  var answers = ['I\'m sorry but I don\'t undersand this', 'Sometimes I think I\'m very stupid because I don\'t understand what you\'re saying'];
  var _msg = answers[Math.floor(Math.random()*answers.length)];
  self._replyToUser(message.user, _msg);      
}
 ...
 *
 */
PlannerBot.prototype._intentHello = function (message) {
  var self = this;
  var hellos = ['Hello', 'Hi', 'Hey'];
  var hello = hellos[Math.floor(Math.random()*hellos.length)];
  var _msg = hello + ", " + self.slack.members[message.user].first_name;
  self._replyToUser(message.user, _msg);   
}
 ...
 *
 */
PlannerBot.prototype._intentSchedule = function (event, message) {
...
PlannerBot.prototype._getIntent = function (event, newMessage, cb) {

  var options = {};
  console.log(event);
  if(typeof event.state !== "undefined" &&
     event.state !== "") {
       options.context = {"state": event.state};
       console.log("Context: " + event.state.white.bold);
  }
   
  wit.captureTextIntent(config.wit.access_token, newMessage, options, function (err, res) {
    if(err) {
      console.log("**** WIT API ERROR ****".red.bold);
      console.log("WIT ACCESS TOKEN = " + config.wit.access_token.red.bold);
      console.log("WIT MESSAGE = " + newMessage.red.bold);
      console.log("***********************\n".red.bold);
      cb(false, err);
    } else {
      cb(true, res);
 ...
 */
PlannerBot.prototype._botTyping = function (channelId) {
  var self = this;
  self.ws.send(JSON.stringify({ type: 'typing', channel: channelId }));
};
