...
PlannerBot.prototype._intentInsult = function (message) {
  var self = this;
  var answers = ['Stop insulting me, I\'ll tell Andrea', 'So rude.', 'Sometimes', 'You are not very nice with me', 'I\'m just a bot'];
...
//{"state": "pending_scheduling"}
  ...
  wit.captureTextIntent(config.wit.access_token, newMessage, options, function (err, res) {
    if(err) {
      console.log("**** DEBUG ****");
      console.log("WIT ACCESS TOKEN = " + config.wit.access_token);
      console.log("WIT MESSAGE = " + newMessage);
      console.log("***************\n");
      cb(false, err);
    } else {
      cb(true, res);
