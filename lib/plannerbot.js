...
var PlannerBot = function Constructor(settings) {
    ...
    this.keywords = {};
    this.keywords.stop = this.settings.keywords || ['cancel', 'stop', 'forget', "don’t worry", "restart", "start again"];
...
PlannerBot.prototype._isStopMessage = function (message) {
    ...
    if (new RegExp(self.keywords.stop.join("|")).test(message.toLowerCase())) {
        // At least one match
        console.log('_isStopMessage (true) => ' + message);
        return true;
    }
    
    console.log('_isStopMessage (false) => ' + message);
    return false;
}
    ...
    event.subject.text !== "") {
          ...
          if(event.action.verb.tense === "present") {
            if(event.subject.text === "I" && event.action.verb.text === "have") {
              ...
              event.summary = event.summary.replace(event.subject.text, ucfirst(self.slack.members[event.message.user]));
              ...
              event.summary = event.summary.replace("I\’m not", ucfirst(self.slack.members[event.message.user])+" is not ");
            } else if(event.subject.text === "I" && event.action.text === "am") {
              // I replace "I'm" with "Andrea"
              event.summary = event.summary.replace(event.subject.text, ucfirst(self.slack.members[event.message.user]));
              // I replace "m" with "is"              
